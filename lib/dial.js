...
(function (window, undefined) {
    ...
    */
    JogDial.Instance = function (el ,opt) {    
      ...
      setTimeout(function(knob) {
        JogDial.utils.triggerEvent(knob, JogDial.CustomEvent.START);
        JogDial.utils.triggerEvent(this.wheel, JogDial.DomEvent.START);
      }, 0, this.knob);
      return this;
    };
    ...
    */
    ...
    function setEvents(self) {
      ...
      */    
      ...
      JogDial.utils.addEvent(W, JogDial.DomEvent.START, start, false);

      
      function start(e) {
        //console.log('internal start');
        //angleTo(self, JogDial.utils.convertClockToUnit(180));    
      }
      ...
      function mouseDragEvent(e) {
        if (self.pressed) {
          ...
          //if(!isValidAngle(self, degree)) return;
          
          radian = modifyAngle(self, JogDial.utils.convertUnitToClock(radian));
          degree = JogDial.utils.convertUnitToClock(radian);
    ...
    function isValidAngle(self, angle) {
      if(angle < self.opt.minDegree)
        return false;
      if(angle > self.opt.maxDegree)
        return false;
      return true;
    }

    function modifyAngle(self, angle) {
      let fixed = Math.round(angle / self.opt.degreeStep) * self.opt.degreeStep;

      return JogDial.utils.convertClockToUnit(fixed);
    }
    ...
    */  
    function angleTo(self, radian) {
      let angle = JogDial.utils.convertUnitToClock(radian);
      if(!isValidAngle(self, angle)) return;
      radian = modifyAngle(self, JogDial.utils.convertUnitToClock(radian));
